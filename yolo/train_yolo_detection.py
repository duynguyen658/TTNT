...
def validate_yolo_dataset(dataset_path: str, data_yaml_path: Optional[str] = None):
    ...
    try:
        ...
        if not train_path.is_absolute():
            train_path = (
                dataset_path.parent / train_path
                if ".." in str(train_path)
                else dataset_path / train_path
            )
        if not val_path.is_absolute():
            val_path = (
                dataset_path.parent / val_path if ".." in str(val_path) else dataset_path / val_path
            )
    ...
    try:
        ...
        for split in ["train", "val", "test"]:
            if split in data_config:
                ...
                split_path = Path(data_config[split])
                labels_path = (
                    split_path.parent / "labels"
                    if split_path.name == "images"
                    else split_path / "labels"
                )
...
5. Train với batch size nhỏ hơn (nếu GPU memory không đủ):
   python train_yolo_detection.py --data data/data.yaml --batch 8
    ...
    parser.add_argument(
        "--epochs", type=int, default=100, help="Số epochs để train (mặc định: 100)"
    )
    parser.add_argument(
        "--imgsz", type=int, default=640, help="Kích thước ảnh (mặc định: 640 cho detection)"
    )
    parser.add_argument("--batch", type=int, default=16, help="Batch size (mặc định: 16)")
    parser.add_argument(
        "--device", type=str, default=None, help="Device (cpu, cuda, 0, 1, etc.). Mặc định: auto"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--project-name",
        type=str,
        default="plant_disease_detection",
