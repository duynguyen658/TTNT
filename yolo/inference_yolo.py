...
class YOLOInference:
    ...
    def predict_single(self, image_path: str, save_path: Optional[str] = None, show: bool = True):
        ...
        results = self.model.predict(
            image_path, conf=self.conf_threshold, iou=self.iou_threshold, verbose=False
        )
    ...
    def evaluate_on_test_set(self, test_images_dir: str, test_labels_dir: Optional[str] = None):
        ...
        print(
            f"   Ảnh có detection: {images_with_detections} ({images_with_detections/total_images:.1%})"
        )
...
5. Tùy chỉnh threshold:
   python inference_yolo.py --model runs/detect/plant_disease_detection/weights/best.pt --image test.jpg --conf 0.5 --iou 0.5
    ...
    parser.add_argument("--image", type=str, default=None, help="Đường dẫn đến ảnh cần dự đoán")
    parser.add_argument("--folder", type=str, default=None, help="Thư mục chứa ảnh cần dự đoán")
    parser.add_argument("--test", type=str, default=None, help="Thư mục test images để đánh giá")
    parser.add_argument("--save", type=str, default=None, help="Thư mục lưu kết quả")
    parser.add_argument("--conf", type=float, default=0.25, help="Confidence threshold (0-1)")
    parser.add_argument("--iou", type=float, default=0.45, help="IoU threshold cho NMS (0-1)")
    parser.add_argument("--no-show", action="store_true", help="Không hiển thị ảnh kết quả")
