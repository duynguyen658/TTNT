...
class DatasetDiagnosisAgent(BaseAgent):
    ...
    async def _analyze_dataset(
        self, df: pd.DataFrame, query: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Phân tích dataset"""
        # Thống kê cơ bản
    ...
    async def _llm_analysis(
        ...
        if not self.client:
            return {"diagnosis": "Không thể phân tích: Thiếu API key", "insights": []}
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Bạn là một chuyên gia nông nghiệp và phân tích dữ liệu bệnh cây trồng với nhiều năm kinh nghiệm. Bạn có khả năng phân tích dataset và đưa ra insights hữu ích về bệnh cây trồng.",
                    },
    ...
    def _extract_insights(self, analysis_text: str) -> list:
        ...
        for sentence in sentences:
            if any(
                ...
                for keyword in ["quan trọng", "đáng chú ý", "phát hiện", "insight"]
            ):
    ...
    def _extract_recommendations(self, analysis_text: str) -> list:
        ...
        for sentence in sentences:
            if any(
                keyword in sentence.lower()
                for keyword in ["nên", "khuyến nghị", "recommend", "nên làm"]
            ):
