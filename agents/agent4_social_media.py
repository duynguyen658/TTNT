...
class SocialMediaSearchAgent(BaseAgent):
    ...
    async def _search_web(self, query: str, keywords: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(3):
                results.append({
                    "source": plant_disease_sources[i] if i < len(plant_disease_sources) else "social_media",
    ...
    async def _summarize_results(self, results: List[Dict[str, Any]], 
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "Bạn là một chuyên gia nông nghiệp và phân tích thông tin từ cộng đồng nông dân. Bạn có khả năng tóm tắt và phân tích thông tin về bệnh cây trồng từ các nguồn mạng xã hội."},
    ...
    def _extract_structured_summary(self, summary_text: str, 
