...
class FinalSynthesisAgent(BaseAgent):
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        synthesis = await self._synthesize_all_results(
            agent1_result, agent2_result, agent3_result, agent4_result,
    ...
    async def _synthesize_all_results(self, agent1: Dict, agent2: Dict, 
                                     agent3: Dict, agent4: Dict,
    ...
    def _detect_conflicts(self, agent2: Dict, agent3: Dict, agent4: Dict) -> List[str]:
        ...
        agent3_diag = str(agent3.get("output", {}).get("llm_diagnosis", {}).get("diagnosis", "")).lower()
    ...
    async def _generate_final_advice(self, synthesis: Dict[str, Any], 
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "Bạn là một chuyên gia nông nghiệp và bệnh học thực vật hàng đầu với nhiều năm kinh nghiệm. Bạn có khả năng tổng hợp thông tin từ nhiều nguồn (hình ảnh, dữ liệu, kinh nghiệm cộng đồng) để đưa ra tư vấn điều trị bệnh cây trồng chính xác, thực tế và hữu ích."},
    ...
    def _simple_advice(self, synthesis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": ["Xem xét kỹ các kết quả từ các agent", "Tham khảo ý kiến chuyên gia nếu cần"],
