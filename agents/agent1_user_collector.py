...
class UserInformationCollector(BaseAgent):
    ...
    def _extract_keywords(self, query: str) -> list:
        ...
        plant_disease_keywords = [
            "bệnh", "cây", "lá", "thân", "rễ", "quả", "hoa", "nấm", "sâu", "côn trùng",
            "vàng", "héo", "thối", "đốm", "cháy", "rụng", "khô", "phấn", "mốc",
            "lúa", "ngô", "cà chua", "ớt", "dưa", "bầu", "bí", "cà tím", "rau"
        ]
        ...
        prioritized_keywords = [w for w in keywords if any(kw in w for kw in plant_disease_keywords)]
    ...
    def _classify_query(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["hình ảnh", "ảnh", "hình", "xem", "phân tích ảnh", "nhận dạng"]):
            return "image_analysis"
        elif any(word in query_lower for word in ["dữ liệu", "dataset", "thống kê", "phân tích dữ liệu", "dữ liệu bệnh"]):
            return "dataset_analysis"
        elif any(word in query_lower for word in ["mạng xã hội", "facebook", "twitter", "social", "diễn đàn", "cộng đồng"]):
            return "social_media"
        elif any(word in query_lower for word in ["tư vấn", "khuyên", "gợi ý", "nên", "điều trị", "chữa", "phòng"]):
            return "consultation"
    ...
    def _check_image_requirement(self, query: str) -> bool:
        ...
        image_keywords = [
            "hình ảnh", "ảnh", "hình", "xem", "phân tích ảnh", "chẩn đoán ảnh",
            "nhận dạng", "ảnh cây", "hình cây", "ảnh lá", "hình lá"
        ]
    ...
    def _check_dataset_requirement(self, query: str) -> bool:
        ...
        dataset_keywords = [
            "dữ liệu", "dataset", "thống kê", "phân tích dữ liệu",
            "dữ liệu bệnh", "dataset bệnh", "thống kê bệnh"
        ]
    ...
    def _check_social_media_requirement(self, query: str) -> bool:
        ...
        social_keywords = [
            "mạng xã hội", "facebook", "twitter", "social media", "cộng đồng",
            "diễn đàn", "nhóm", "kinh nghiệm", "người dùng", "nông dân"
        ]
        # Nếu câu hỏi về bệnh cây, nên tìm kiếm thêm trên mạng xã hội
        has_disease_query = any(word in query.lower() for word in ["bệnh", "cây", "tư vấn", "kinh nghiệm"])
    ...
    async def _enrich_with_llm(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "Bạn là một chuyên gia nông nghiệp AI, chuyên thu thập và phân tích thông tin về bệnh cây trồng từ người dùng."},
